...
# Modern color palette with gradients - deeper colors for better contrast on white background
def create_gradient(base_color, darken_factor=0.3):
...
colors = {
    'input': create_gradient('#2980b9'),         # Deeper Blue
    'preprocess': create_gradient('#8e44ad'),    # Deeper Purple
    'regression': create_gradient('#27ae60'),    # Deeper Green
    'impact': create_gradient('#c0392b'),        # Deeper Red
    'fee': create_gradient('#d35400'),           # Deeper Orange
    'integration': create_gradient('#8e44ad'),   # Dark Purple
    'output': create_gradient('#16a085'),        # Deeper Teal
...
def draw_modern_box(x, y, width, height, text, color_map, alpha=1.0, fontsize=12, zorder=10):
...
def add_detail_box(x, y, width, height, text, color='#f5f5f5', fontsize=9, alpha=0.95, border_color='#777777'):
    rect = Rectangle((x, y), width, height, facecolor=color, edgecolor=border_color, 
...
# IMPROVED LAYOUT WITH MORE SPACING TO PREVENT OVERLAPPING
...
# Regression Models - Add more horizontal spacing between model boxes
...
# Integration - Further down to prevent overlapping
draw_modern_box(4, 3.7, 6, 0.8, "Cost Integration & Analysis", colors['integration'], fontsize=14)

# Output - Further down to prevent overlapping
draw_modern_box(4, 2.3, 6, 0.8, "Execution Cost & Market Impact Estimate", colors['output'], fontsize=14)
...
draw_modern_arrow((2.5, 6.2), (5, 4.5), curve=0.2)
draw_modern_arrow((7, 6.2), (7, 4.5))
draw_modern_arrow((11.5, 6.2), (9, 4.5), curve=-0.2)

# Arrow from integration to output
draw_modern_arrow((7, 3.7), (7, 3.1))
...
"""
add_detail_box(0.2, 4.8, 4.6, 1.2, regr_detail, color='#f8f8f8', border_color='#999999')

# Almgren-Chriss Model
impact_detail = """
AlmgrenChriss: Market impact calculation
- Temporary impact: σ·γ·√(τ/V)·(q/Q)^d
- Permanent impact: κ·q/V
- Optimal execution trajectory
- Risk-adjusted cost minimization
"""
add_detail_box(4.7, 4.8, 4.6, 1.2, impact_detail, color='#f8f8f8', border_color='#999999')
...
"""
add_detail_box(9.2, 4.8, 4.6, 1.2, fee_detail, color='#f8f8f8', border_color='#999999')
...
"""
add_detail_box(1.2, 7.2, 4, 0.4, features_detail, color='#f0f0f0', border_color='#999999')
...
"""
add_detail_box(8.8, 7.2, 4, 0.4, market_detail, color='#f0f0f0', border_color='#999999')
...
"""
add_detail_box(8.5, 3.3, 5, 0.6, integration_detail, color='#f0f0f0', border_color='#999999')
